  ...
  ngOnInit() {
    ...
    if (this.es_admin) {
      this.usuarioService.admin.subscribe({
        next: (admin) => {
          this.admin = admin;
          this.eventoService.getAllEventos().subscribe({ next: (eventos) => this.eventos = eventos });
        }
      });
    } else {
      this.usuarioService.usuario.subscribe({
        next: (usuario) => {
          ...
          console.log('NO ES ADMIN' + this.usuario.id)
          this.eventoService.getEventosDeUsuario(this.usuario.id!).subscribe({ next: (eventos) => this.eventos = eventos });
        }
      });
    }
